...
function RemoteProfiler(cb) {
  Cu.import("resource://gre/modules/devtools/dbg-client.jsm");
  this._client = new DebuggerClient(debuggerSocketConnect("localhost", 6000));
...
RemoteProfiler.prototype = {
  ...
  shutdown: function profiler_shutdown() {
    Profiler.prototype.shutdown.call(this);
...
let FirefoxAppWrapper = {
  ...
  init: function ff_init() {
      ...
      panel.port.on("rdpconnect", function(text) {
          var p = profiler;
          profiler = null;
          if (p) p.shutdown();
          appWrapper.initProfiler(RemoteProfiler, function() { });
      });
      panel.port.on("rdpdisconnect", function(text) {
          var p = profiler;
          profiler = null;
          if (p) p.shutdown();
          appWrapper.initProfiler(Profiler, function() { });
      });
